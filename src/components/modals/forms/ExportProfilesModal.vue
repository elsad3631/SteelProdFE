    ...
    <div class="modal-dialog modal-dialog-centered mw-650px">
      ...
      <div class="modal-content">
        ...
        <div class="modal-header">
          <!--begin::Modal title-->
          <h2 class="fw-bold">Export Profili</h2>
          <!--end::Modal title-->
    ...
    const submit = () => {
      ...
      formRef.value.validate((valid: boolean) => {
        if (valid) {
          ...
          ApiService.get(`Profiles/${api.value}?fromName=${formData.value.fromName}&toName=${formData.value.toName}`, "blob")
            .then((response) => {

              const url = URL.createObjectURL(new Blob([response.data]))
              const link = document.createElement('a');
              link.href = url;
              link.setAttribute('download', 'export profili' + output.value);
