        ...
        <div class="modal-body scroll-y mx-5 mx-xl-15 my-7">
          ...
          <el-form @submit.prevent="submit()" :model="formData" :rules="rules" ref="formRef">
            ...
            <div class="fv-row mb-10">
              <!--begin::Label-->
              <label class="fs-5 fw-semobold form-label mb-5">Seleziona il range da data a data:</label>
              <!--end::Label-->

              <!--begin::Input-->
              <div class="input-group mb-5">
                <select class="form-select" aria-label="Select example" v-model="formData.fromDate">
                  <option v-for="item in alphabet" :key="item.name" :value="item.name">{{ item.name }}</option>
                </select>
                <span class="input-group-text">-</span>
                <select class="form-select" aria-label="Select example" v-model="formData.toDate">
                  <option v-for="item in alphabet" :key="item.name" :value="item.name">{{ item.name }}</option>
                </select>
              </div>
              <!--end::Input-->
    ...
    const submit = () => {
      ...
      formRef.value.validate((valid: boolean) => {
        if (valid) {
          ...
          ApiService.get(`GoodReceipt/${api.value}?fromDate=${formData.value.fromDate}&toDate=${formData.value.toDate}`, "blob")
            .then((response) => {
