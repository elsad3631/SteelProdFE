<template>
        ...
        <div class="card-header border-0 pt-6">
            ...
            <div class="card-toolbar">
                ...
                <div v-if="selectedIds.length === 0" class="d-flex justify-content-end"
                    data-kt-payment-table-toolbar="base">
                    <!--begin::Add payment-->
                    <button type="button" class="btn btn-primary" data-bs-toggle="modal"
                        data-bs-target="#kt_modal_add_type">
                        <KTIcon icon-name="plus" icon-class="fs-2" />
                        Aggiungi tipologia pagamento
                    </button>
                    <!--end::Add payment-->
                </div>
                <!--end::Toolbar-->
                <!--begin::Group actions-->
                <div v-else class="d-flex justify-content-end align-items-center" data-kt-payment-table-toolbar="selected">
                ...
                <!--end::Group actions-->
                <!--begin::Group actions-->
                <div class="d-flex justify-content-end align-items-center d-none" data-kt-payment-table-toolbar="selected">
                    <div class="fw-bold me-5">
                        <span class="me-2" data-kt-payment-table-select="selected_count"></span>Selezionati
                    </div>
                    <button type="button" class="btn btn-danger" data-kt-payment-table-select="delete_selected">
                        Elimina Selezionati
                    </button>
                </div>
                <!--end::Group actions-->
    ...
    <UpdatePaymentTypeModal :id="selectedId" :name="selectedName" @formUpdateSubmitted="getItems('')"></UpdatePaymentTypeModal>
    <AddPaymentTypeModal @formAddSubmitted="getItems('')"></AddPaymentTypeModal>
</template>
...
import UpdatePaymentTypeModal from "@/components/modals/forms/typologies/payment-type/UpdatePaymentTypeModal.vue";
import AddPaymentTypeModal from "@/components/modals/forms/typologies/payment-type/AddPaymentTypeModal.vue";
...
export default defineComponent({
    ...
    setup() {
        ...
        let tableData = ref<IPaymentType[]>([]);

        async function getItems(filterRequest: string) {
            tableData.value = await getPaymentTypes(filterRequest);
            loading.value = false;
        };
        ...
        const deleteItem = (id: number) => {
            loading.value = true;
            ApiService.post(`PaymentType/Delete?id=${id}`, {})
                .then(() => {
