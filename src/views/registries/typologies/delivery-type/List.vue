<template>
        ...
        <div class="card-header border-0 pt-6">
            ...
            <div class="card-toolbar">
                ...
                <div v-if="selectedIds.length === 0" class="d-flex justify-content-end"
                    data-kt-delivery-table-toolbar="base">
                    <!--begin::Add delivery-->
                    <button type="button" class="btn btn-primary" data-bs-toggle="modal"
                        data-bs-target="#kt_modal_add_type">
                        <KTIcon icon-name="plus" icon-class="fs-2" />
                        Aggiungi tipologia consegna
                    </button>
                    <!--end::Add delivery-->
                </div>
                <!--end::Toolbar-->
                <!--begin::Group actions-->
                <div v-else class="d-flex justify-content-end align-items-center" data-kt-delivery-table-toolbar="selected">
                ...
                <div class="d-flex justify-content-end align-items-center d-none" data-kt-delivery-table-toolbar="selected">
                    <div class="fw-bold me-5">
                        <span class="me-2" data-kt-delivery-table-select="selected_count"></span>Selezionati
                    </div>
                    <button type="button" class="btn btn-danger" data-kt-delivery-table-select="delete_selected">
                        Elimina Selezionati
                    </button>
                </div>
                <!--end::Group actions-->
    ...
    <UpdateDeliveryTypeModal :id="selectedId" :name="selectedName" @formUpdateSubmitted="getItems('')"></UpdateDeliveryTypeModal>
    <AddDeliveryTypeModal @formAddSubmitted="getItems('')"></AddDeliveryTypeModal>
</template>
...
import UpdateDeliveryTypeModal from "@/components/modals/forms/typologies/delivery-type/UpdateDeliveryTypeModal.vue";
import AddDeliveryTypeModal from "@/components/modals/forms/typologies/delivery-type/AddDeliveryTypeModal.vue";
...
export default defineComponent({
    ...
    setup() {
        ...
        let tableData = ref<IDeliveryType[]>([]);

        async function getItems(filterRequest: string) {
            tableData.value = await getDeliveryTypes(filterRequest);
            loading.value = false;
        };
        ...
        const deleteItem = (id: number) => {
            loading.value = true;
            ApiService.post(`DeliveryType/Delete?id=${id}`, {})
                .then(() => {
