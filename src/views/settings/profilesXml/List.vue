            ...
            <Datatable @on-sort="sort" @on-items-select="onItemSelect" :data="tableData" :header="tableHeader"
                ...
                <template v-slot:actions="{ row: item }">
                    <button class="btn btn-light-info me-1" data-bs-toggle="modal"
                        ...
                        @click="showUpdateModal(item.id, item.name, item.active)">Dettagli</button>
                    <button @click="deleteItem(item.id)" class="btn btn-light-danger me-1">Elimina</button>
                </template>
            </Datatable>
        </div>
    </div>
    <UpdateProfileXmlModal :id="selectedId" :name="selectedName" :active="selectedActive" @formUpdateSubmitted="getItems('')"></UpdateProfileXmlModal>
    <AddProfileXmlModal @formAddSubmitted="getItems('')"></AddProfileXmlModal>
</template>
...
import UpdateProfileXmlModal from "@/components/modals/forms/UpdateProfileXmlModal.vue";
import AddProfileXmlModal from "@/components/modals/forms/AddProfileXmlModal.vue";
import arraySort from "array-sort";
import { MenuComponent } from "@/assets/ts/components";
import ApiService from "@/core/services/ApiService";
import Loading from "@/components/kt-datatable/table-partials/Loading.vue"
import { getProfilesXml } from "@/core/data/profilesXml";
import type { IProfileXml } from "@/core/data/profilesXml";
import Swal from "sweetalert2/dist/sweetalert2.js";

export default defineComponent({
    name: "Profile-xml-list",
    components: {
        Datatable,
        UpdateProfileXmlModal,
        AddProfileXmlModal,
        Loading
    },
    setup() {
        ...
        let tableData = ref<IProfileXml[]>([]);

        async function getItems(filterRequest: string) {
            tableData.value = await getProfilesXml(filterRequest);
            loading.value = false;
        };
        ...
        const deleteItem = (id: number) => {
            loading.value = true;
            ApiService.post(`ProfileXml/Delete?id=${id}`, {})
                .then(() => {
        ...
        function showUpdateModal(id, name, active) {
            selectedId.value = id;
